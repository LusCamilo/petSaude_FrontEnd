...
export const PetConfig = () => {
	...
	const editarPetizinho = async (id, infosPet) => {
		await Notifications.confirmOrCancel(`Deseja atualizar as informações do pet ${infos.name}?`, async result => {
			if (result.isConfirmed) {
				const {response} = await petUpdate(id, infosPet);

				if (response === "Item atualizado com sucesso no Banco de Dados") {
					await Notifications.success('Informações atualizadas com sucesso')
					document.location.href = "/profile/configuration";
				} else {
					await Notifications.error('Não foi possível realizar as alterações', 'Tente novamente')
	...
	return (
		<section>
			...
			<main className="static w-full">
				...
				<div
					className="w-full h-full border-none sm:border-solid border-2 rounded-lg border-black flex flex-col gap-5 md:p-10">
					...
					<div className="flex flex-col sm:flex-row justify-between pr-20">
						...
						<div className="w-full sm:w-1/3 h-fit flex justify-end content-center">
							<button
								className='w-fit px-14 h-14 flex-row justify-center items-center cursor-pointer gap-4 bg-[#ECECEC] rounded-full drop-shadow-lg hidden md:flex text-2xl'
								onClick={() => {
