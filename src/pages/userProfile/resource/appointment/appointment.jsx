      ...
      if (decoded.id) {
        ...
        if (pets == "Não foram encontrados registros no Banco de Dados") {
          await Notifications.error(
            "Você não tem pets!",
            "Crie um pet para fazer uma consulta!"
          );
          setTimeout(function () {
    ...
    <section id="buttonCanceled" className=" w-full h-full p-10">
      <form className=" flex justify-start w-full">
        <div className="w-full">
          <h1 className="flex justify-start text-3xl md:text-5xl font-normal">
            Selecione o animal
          </h1>
          <div className="flex justify-between md:mt-5 w-full h-56 gap-20">
            <div
              className={`${ThereArePets} flex-col w-1/2 md:p-5 md:pt-3 overflow-x-auto max-h-60 border-2 rounded-xl shadow-lg  bg-slate-100`}
            >
              {petsAll.map((pet) => {
            ...
            <div className="hidden md:flex w-1/2 justify-center items-center gap-5">
              <img
                className="w-44 h-44 rounded-full flex justify-center"
                src={petImage}
                alt={"Pet"}
              ></img>
              <div className="flex flex-col justify-center">
                <label className="flex w-fit">
                  <input
                    ...
                    name="firstName"
                    value={petsName}
                    className="bg-transparent text-black border-none md:text-3xl h-fit font-normal"
                  />
                </label>
                <label className="flex w-fit">
                  <input
                    ...
                    name="firstName"
                    value={petsEspecie}
                    className="bg-transparent border-none md:text-4xl h-fit text-[#A9A9A9]"
                  />
                </label>
              </div>
            </div>
        ...
        <div className=" w-full">
          <div className="pt-10">
            <h2 className="hidden md:flex md:text-4xl font-normal">
              Informações
            </h2>
            <h2 className="flex justify-center text-3xl md:hidden md:text-5xl font-semibold">
              Data e hora
            </h2>
            ...
            <div className="w-full mt-5">
              <label
                htmlFor="descricao"
                className=" mb-2 flex justify-start pt-5"
              >
                Descrição:
              </label>
              <textarea
          ...
          <div className="pt-5  ">
            <div className="flex mt-5 justify-between gap-5"></div>
          </div>
        </div>
      </form>
    </section>
