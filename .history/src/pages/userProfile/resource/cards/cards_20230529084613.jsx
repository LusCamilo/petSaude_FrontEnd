...
export const Cards = (props) => {
	...
	const infosCards = async () => {
		...
		if (props.isVet == true) {
			...
			return response.response.ratings
		} else {
				...
				<div className='flex items-center pl-14 md:pl-0 justify-between'>
					...
					<div className='md:flex overflow-x-auto scroll-smooth md:gap-2 md:pr-[45%] w-full ' ref={carrossel}>
						{petOrRating.map((item) => {
							return <Rating id={item.id} personImage={props.personImage} userName={props.userName} score={item.score} text={item.description} whenCreated={item.createdAt}/>
						})}
						
					</div>
					props.isVet = false && props.Id <IoIosArrowForward className='text-5xl cursor-pointer' onClick={handleRightClick}/>
				</div>
			</div>
		);
	} else {
