				...
				<div className='flex items-center pl-14 md:pl-0 justify-between'>
					...
					<div className='md:flex overflow-x-auto scroll-smooth md:gap-2 md:pr-[45%] w-full ' ref={carrossel}>
						{petOrRating.map((item) => {
							return <Rating key={item.id} id={item.id} id={item.id} id={item.id} personImage={props.personImage} userName={props.userName} score={item.score} text={item.description} whenCreated={item.createdAt}/>
						})}
