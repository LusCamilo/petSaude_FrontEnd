...
export const Cards = (props) => {
	...
	const infosCards = async () => {
		...
		if (props.isVet == true) {
			...
			console.log(response.response.ratings[0]);
				...
				<div className='flex items-center pl-14 md:pl-0 justify-between'>
					...
					<div className='md:flex overflow-x-auto scroll-smooth md:gap-2 md:pr-[45%] w-full ' ref={carrossel}>
						{petOrRating.map((item) => {
							return <Rating key={item.id} id={item.id} clientId={item.clientId} idVet={props.id} personImage={props.personImage} userName={props.userName} score={item.score} text={item.description} whenCreated={item.createdAt}/>
						})}
