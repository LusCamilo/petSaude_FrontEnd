	...
	return (
		<div id={props.id} className='w-80 h-48 border border-solid border-[#CAC4D0] rounded-2xl flex flex-none flex-col'>
			<div className='h-1/4 flex flex-row justify-between items-center px-5'>
				...
				<Dropdown
				    button={
				   <button className={`h-6 ${props.clientId != idPerson || props.idVet == idPerson ? 'flex' : 'hidden'}`}>lassName="bg-brand-500 hover:bg-brand-600 active:bg-brand-700 dark:bg-brand-400 dark:hover:bg-brand-300 dark:active:bg-brand-200 rounded-xl px-5 py-3 text-base font-medium text-white transition duration-200 dark:text-white">
				   	<img src="https://static.thenounproject.com/png/1919184-200.png" alt="" className="h-6" />n Menu
				   </button>
				    }
				    children={
